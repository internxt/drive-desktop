...
$testDllPath = Join-Path $PSScriptRoot "build\test-install\resources\context-menu\internxt_context_menu.dll"
...
$artifactsExist =
  ...
  (Test-Path -LiteralPath $testDllPath)
...
if (-not $requiresSetup) {
  ...
  $oldestArtifactWriteTime = (@($msixPath, $testDllPath) | Get-Item | Measure-Object LastWriteTimeUtc -Minimum).Minimum
