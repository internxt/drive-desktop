...
describe('File Overrider', () => {
  it('throws an error if no file is founded with the given fileId', async () => {
    ...
    const overrider = new FileOverrider(repository, rfs, eventBus);
    ...
    try {
      await overrider.run(file.id, updatedContentsId.value, updatedSize.value);
  ...
  it('calls the override method with the updated contentsId and size updated', async () => {
    ...
    repository.searchByIdMock.mockResolvedValueOnce(file);
  ...
  it('emits the FileOverridden domain event when successfully overridden ', async () => {
    ...
    await overrider.run(file.id, updatedContentsId, FileSizeMother.primitive());
