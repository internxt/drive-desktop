...
import { FileExplorerFiles, FileExplorerFolders, FileExplorerItem } from '../sync-items-by-checkpoint/load-in-memory-paths';
...
export async function deleteItemPlaceholder({ ctx, type, remote, locals }: Props) {
  try {
    const local = (locals as Map<string, FileExplorerItem>).get(remote.uuid);
