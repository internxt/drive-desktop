...
import { createMailBridgeManager } from '@/backend/features/mail-bridge/manager/create-mail-bridge-manager';
import { getWidget } from '../../../../apps/main/windows/widget';

const mailBridgeManager = createMailBridgeManager({
...
export function getMailBridgeStatus() {
  return mailBridgeManager.getStatus();
}
export async function startMailBridge({ ctx }: { ctx: AuthContext }) {
  ...
  const result = await mailBridgeManager.start(session.data);
...
export async function stopMailBridge() {
  const result = await mailBridgeManager.stop();
