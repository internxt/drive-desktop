...
import { getWebCachePaths } from './get-cache-file-paths';
...
export async function generateWebCacheReport(): Promise<CleanerSection> {
  const paths = getWebCachePaths();
  let allItems: CleanableItem[] = [];

  const scanSubSectionPromises = [
    CleanerModule.scanDirectory({
      ctx: cleanerCtx,
      absolutePath: paths.chrome,
      customFileFilter: CleanerModule.isSafeWebBrowserFile,
    }),

    CleanerModule.scanFirefoxCacheProfiles({ ctx: cleanerCtx, firefoxCacheDir: paths.firefox }),

    CleanerModule.scanDirectory({
      ctx: cleanerCtx,
      absolutePath: paths.edge,
      customFileFilter: CleanerModule.isSafeWebBrowserFile,
    }),

    CleanerModule.scanDirectory({
      ctx: cleanerCtx,
      absolutePath: paths.edgeIECache,
      customFileFilter: CleanerModule.isSafeWebBrowserFile,
    }),
  ];
