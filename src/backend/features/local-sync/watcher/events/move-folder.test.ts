...
describe('move-folder', () => {
  ...
  beforeEach(() => {
    ...
    getByNameMock.mockResolvedValue({ data: { uuid: 'uuid' as FolderUuid, parentUuid: 'parentUuid' } });
  ...
  it('should perform move request and ignore unlink event', async () => {
    ...
    call(persistMoveFolderMock).toStrictEqual({ parentUuid: 'parentUuid', path: folder2, uuid: 'uuid', workspaceToken: undefined });
  });
});
