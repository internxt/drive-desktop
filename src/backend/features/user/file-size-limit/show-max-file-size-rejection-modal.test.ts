...
describe('showMaxFileSizeRejectionModal', () => {
  ...
  function closeWindow(windowMock: ReturnType<typeof mockBrowserWindow> | undefined) {
    const closedHandler = windowMock?.on.mock.calls.find(([eventName]) => eventName === 'closed')?.[1] as
      | (() => void)
      | undefined;
  ...
  it('should create a shorter modal window when upgrade cta is hidden', async () => {
    mockBrowserWindow();
