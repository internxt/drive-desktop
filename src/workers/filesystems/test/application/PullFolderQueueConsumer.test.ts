...
describe('Pull Folder Queue Consumer', () => {
	...
	beforeEach(() => {
		...
		originFS.mockGetFolderData.mockImplementation(() => Promise.resolve({ modtime: 50 }));
	});

	it('creates all the folders that recives', async () => {
		const folders = ['folderA', 'folderB', 'folderC'];

		const consumer = new PullFolderQueueConsumer(originFS, destinationFS, eventEmiter);
	...
	it('pulls the folders in order from the more superficial to the most nested', async () => {
		...
		destinationFS.assertFolderHasBeenPulledBeforeThan('folderC', 'folderC/subfolderA');
	});

	it('does not pull a folder from a more nested level if does not finish the previous', async () => {
		const folders = ['folderA/subfolderA', 'folderA/subfolderB/sub-subfolderA', 'folderA'];
