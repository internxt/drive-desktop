...
import { DriveFoldersCollection } from '../../main/database/collections/driveFoldersCollection';
import { DriveFilesCollection } from '../../main/database/collections/driveFilesCollection';
...
export function getRemoteFilesystem({
  ...
  async function getFilesFromLocalDB(): Promise<ServerFile[]> {
    const driveFilesCollection = new DriveFilesCollection();

    await driveFilesCollection.connect();

    const { success, result } = await driveFilesCollection.getAll();

    if (!success)
      throw new Error('Retrieve files from localDB operation not successful');

    return result.map<ServerFile>((item) => {
      const mappedItem: ServerFile = {
        bucket: item.bucket,
        createdAt: item.createdAt,
        encrypt_version: 'deprecated_field',
        fileId: item.fileId,
        folderId: item.folderId,
        id: item.id,
        modificationTime: item.updatedAt,
        name: item.plainName,
        size: item.size,
        type: item.type,
  ...
  async function getFoldersFromLocalDB(): Promise<ServerFolder[]> {
    const driveFoldersCollection = new DriveFoldersCollection();

    await driveFoldersCollection.connect();

    const { success, result } = await driveFoldersCollection.getAll();

    if (!success)
      throw new Error('Retrieve folders from localDB operation not successful');

    return result.map<ServerFolder>((item) => {
      const mappedItem: ServerFolder = {
        bucket: item.bucket,
  ...
  async function getTree(retrieveFrom: 'server' | 'local-db'): Promise<{
    ...
    if (retrieveFrom === 'local-db') {
      // We should ensure here that this files are in sync with remote, the way to
      // do this is by calling RemoteSyncManager.startRemoteSync, waiting for it to finish
      // and then reading the LocalDB, however could be possible that we call this before,
      // we should avoid that
      const files = await getFilesFromLocalDB();
      const folders = await getFoldersFromLocalDB();
  ...
  return {
    ...
    async getCurrentListing() {
      const tree = await getTree('server');
