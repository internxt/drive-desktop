...
import { AllStatusesTraverser } from '../../items/application/AllStatusesTraverser';

export class SyncPlaceholders {
  constructor(
    private readonly allStatusesTraverser: AllStatusesTraverser,
  ...
  async run() {
    const tree = await this.allStatusesTraverser.run();
