...
export function convertActionsToQueues(
  actions: Array<Action<'FILE' | 'FOLDER'>>
): QeuesByItemKind {
  return {
    file: extractNamesFromActionsQueues(
      filterQueueForItemKind('FILE')(actions)
    ),
    folder: extractNamesFromActionsQueues(
      filterQueueForItemKind('FOLDER')(actions)
    ),
  };
}
