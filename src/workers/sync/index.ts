...
interface IpcRenderer {
	send<U extends keyof SyncEvents>(event: U, ...args: Parameters<SyncEvents[U]>): void;
...
async function setUp() {
	const { localPath, tmpPath, folderId } = await ipcRenderer.invoke('get-sync-details');
	...
	sync.on('CHECKING_LAST_RUN_OUTCOME', () => Logger.log('Checking last run outcome'));
	...
	sync.on('ERROR_RENAMING_FILE', (oldName, newName, kind, errorName, errorDetails) => {
		Logger.error(
	...
	sync.on('FOLDER_DELETED', (name, kind) => Logger.debug(`Deleted folder ${name} in ${kind}`));
	...
	try {
		...
		if (err instanceof ProcessFatalError) {
			Logger.error(
				`Sync fatal error (${err.name}), details: ${JSON.stringify(err.details, null, 2)}`
			);
			ipcRenderer.send('SYNC_FATAL_ERROR', err.name as ProcessFatalErrorName);
		} else {
			Logger.error('Completely unhandled sync fatal error', JSON.stringify(err, null, 2));
