...
import Process, { ProcessEvents, ProcessResult, SuccessfulProcessResult } from '../process';
import {
	EnqueuedSyncActions,
	FileSystem,
	Listing,
...
import { Listing as NewListing, LocalListing, RemoteListing } from './Listings/domain/Listing';
...
class Sync extends Process {
	...
	async run(): Promise<ProcessResult> {
		...
		const { pullFromRemote: pullFoldersFromRemote, pullFromLocal: pullFoldersFromLocal } =
			queues.folder;
		...
		const remoteFolderPullConsumer = new PullFolderQueueConsumer(this.local, this.remote, this);
		const localFolderPullConsumer = new PullFolderQueueConsumer(this.remote, this.local, this);
		...
		Logger.debug('Queue delete from remote', deleteInRemoteNotIncludedOnFolderDeletion);

		await Promise.all([
			this.consumeDeleteQueue(deleteInLocal, this.local),
			this.consumeDeleteQueue(deleteInRemoteNotIncludedOnFolderDeletion, this.remote),
		]);
	...
	private async resync(): Promise<ProcessResult> {
		...
		await Promise.all([
			this.consumePullQueue(pullFromLocal, this.oldFileSystems.local, this.remote),
			this.consumePullQueue(pullFromRemote, this.oldFileSystems.remote, this.local),
		]);
...
declare interface Sync {
	...
	emit<U extends keyof SyncEvents>(event: U, ...args: Parameters<SyncEvents[U]>): boolean;
}
