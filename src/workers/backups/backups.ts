...
class Backups extends Process {
	...
	async run(): Promise<ProcessResult> {
		...
		this.emit('ACTION_QUEUE_GENERATED', pullFromRemote.length + deleteInRemote.length);
		...
		const smallFileChunks = _.chunk(
			smallFiles,
			getChunkSize(smallFiles.length, Backups.NUMBER_OF_PARALLEL_QUEUES_FOR_SMALL_FILES)
		);
		...
		const mediumFiles = pullFromRemote.filter((name) => {
			...
			return size > Backups.MAX_SMALL_FILE_SIZE && size <= Backups.MAX_MEDIUM_FILE_SIZE;
		});
		Logger.debug('Medium files: ', mediumFiles);

		const mediumFileChunks = _.chunk(
			mediumFiles,
			getChunkSize(mediumFiles.length, Backups.NUMBER_OF_PARALLEL_QUEUES_FOR_MEDIUM_FILES)
		);
		...
		const bigFileChunks = _.chunk(
			bigFiles,
			getChunkSize(bigFiles.length, Backups.NUMBER_OF_PARALLEL_QUEUES_FOR_BIG_FILES)
		);
		...
		await Promise.all([...bigFileQueues, this.consumeDeleteQueue(deleteInRemote, this.remote)]);
...
declare interface Backups {
	...
	emit<U extends keyof BackupsEvents>(event: U, ...args: Parameters<BackupsEvents[U]>): boolean;
}
