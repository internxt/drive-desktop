...
abstract class Process extends EventEmitter {
	constructor(protected readonly local: FileSystem, protected readonly remote: FileSystem) {
		super();
	}
	...
	protected async consumePullQueue(
		queue: string[],
		destFs: Pick<FileSystem | NewFileSystem<PartialListing>, 'pullFile' | 'kind'>,
		...
		for (const name of queue) {
			...
			try {
				...
				const id = await destFs.pullFile(name, source, progressCallback, abortController.signal);
				...
				const syncError =
					err instanceof ProcessError
						...
						: new ProcessError(
								'UNKNOWN',
								createErrorDetails(err, 'Pulling file', `name: ${name}, kind: ${destFs.kind}`)
						  );
				this.emit(
					'ERROR_PULLING_FILE',
					name,
	...
	protected async consumeDeleteQueue(queue: string[], fileSystem: FileSystem): Promise<void> {
		for (const name of queue) {
			...
			try {
				...
				const syncError =
					err instanceof ProcessError
						...
						: new ProcessError(
								'UNKNOWN',
								createErrorDetails(err, 'Deleting file', `name: ${name}, kind: ${fileSystem.kind}`)
						  );
				this.emit(
					'ERROR_DELETING_FILE',
					name,
	...
	protected async consumeDeleteFolderQueue(queue: string[], fileSystem: FileSystem): Promise<void> {
		for (const name of queue) {
	...
	protected getListingsDiff(local: Listing, remote: Listing): ListingsDiff {
		...
		for (const [localName, { modtime: localModtime }] of Object.entries(local)) {
	...
	protected async getCurrentListings(options: { emitErrors: boolean }): Promise<{
	...
	protected emitReadingMetaErrors(errors: ReadingMetaErrorEntry[], kind: FileSystemKind) {
		errors.forEach((entry) => {
			this.emit('ERROR_READING_METADATA', entry.name, kind, entry.errorName, entry.errorDetails);
		});
	}
...
export interface ProcessEvents {
	 ...
	 */
	FILE_PULLED: (name: string, fileSystemKind: FileSystemKind, fileId: number | undefined) => void;
	/**
