...
describe('overwrite-dangled-files', () => {
  ...
  const props = mockProps<typeof overwriteDangledContents>({
    ctx: { contentsDownloader },
    dangledFiles: [{ absolutePath: abs('/file.txt') }],
  });
  ...
  it('should update isDangledStatus if contents is found', async () => {
    ...
    // When
    await overwriteDangledContents(props);
    // Then
    calls(props.ctx.logger.debug).toMatchObject([{ msg: 'Downloading dangled file' }, { msg: 'Dangled file contents found' }]);
  ...
  it('should update contents id if contents not found', async () => {
    ...
    await overwriteDangledContents(props);
    // Then
    call(props.ctx.logger.debug).toMatchObject({ msg: 'Downloading dangled file' });
    call(props.ctx.logger.warn).toMatchObject({ msg: 'Dangled file contents not found' });
