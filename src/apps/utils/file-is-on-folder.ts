...
const extractFilePath = (fileName: string): string => fileName.substring(0, fileName.lastIndexOf('/'));
...
export function itemIsInFolder(foldersName: Array<string>): (fileName: string) => boolean {
  const sanitazedFolderPaths = foldersName.map((foldersPath: string) => sanitazeRelativePath(foldersPath));
