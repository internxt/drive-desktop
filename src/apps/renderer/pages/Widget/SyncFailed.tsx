  ...
  return (
    <div className="relative flex flex-1 flex-col items-center justify-center space-y-5 px-8">
      ...
      <div className="flex flex-col items-center space-y-1 text-center">
        <h1 className="text-lg font-medium leading-tight text-gray-100">{t('widget.sync-error.title')}</h1>
        <h3 className="text-sm leading-base text-gray-60">{t('widget.sync-error.message')}</h3>
      </div>

      <Button variant="secondary" onClick={handleRetrySync}>
        {t('widget.sync-error.button')}
      </Button>
    </div>
  );
}
