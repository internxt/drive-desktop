...
const stateIsError = (maybe: unknown): maybe is ErrorReportRequestState =>
  typeof maybe === 'string' && posibleErrorStates.includes(maybe as ErrorReportRequestState);

const errorMessages: Record<ErrorReportRequestState, string> = {
  ERROR: 'We could not send your request, make sure you are connected to the internet',
  TOO_MANY_REPORTS: 'Reached reporting errors day limit',
};

export function ReportModal({ data, onClose }: { data: Pick<ProcessIssue, 'errorName'> | null; onClose: () => void }) {
  ...
  const supportParagraph = (
    <>
      ...
      <span className="cursor-pointer text-primary" onClick={() => handleOpenURL('https://help.internxt.com')}>
        help.internxt.com
      </span>
          ...
          <Transition.Child
            ...
            <div className="my-6 inline-block w-full max-w-md transform overflow-hidden rounded-xl bg-surface p-5 text-left align-middle shadow-xl transition-all dark:bg-gray-1">
              <div className="flex flex-col space-y-2">
                <Dialog.Title as="h3" className="font-medium leading-6 text-gray-100">
                  {dialogTitle}
                </Dialog.Title>

                <p className="text-sm text-gray-60">{phase === 'INITIAL' ? errorDescription : supportParagraph}</p>

                {phase === 'REPORTING' && (
                  <div className="flex h-40 flex-col space-y-1.5">
                    <p className="text-sm font-medium text-gray-100">{translate('issues.report-modal.user-comments')}</p>

                    <TextArea value={userComment} onChange={(e) => setUserComment(e.target.value)} customClassName="flex-1" resize="none" />

                    {stateIsError(requestState) && <p className="text-sm text-red">{errorMessages[requestState]}</p>}
                ...
                <div className="flex shrink-0 items-center justify-end space-x-2">
                  <Button variant="secondary" size="sm" onClick={() => (phase === 'INITIAL' ? onClose() : setPhase('INITIAL'))}>
                    {translate(phase === 'INITIAL' ? 'issues.report-modal.actions.close' : 'issues.report-modal.actions.cancel')}
                  </Button>
                  ...
                  <Button
                    ...
                    variant="primary"
                    size="sm"
                    onClick={() => (phase === 'INITIAL' ? setPhase('REPORTING') : handleSubmit())}>
                    {phase === 'INITIAL' ? translate('issues.report-modal.actions.report') : translate('issues.report-modal.actions.send')}
                  </Button>
                </div>
              </div>
            </div>
