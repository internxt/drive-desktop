...
describe('ScanStatistics', () => {
  ...
  it('renders the component with correct content', () => {
    ...
    expect(mockTranslate).toHaveBeenCalledWith(
      'settings.antivirus.scanProcess.scannedFiles'
    );
    expect(mockTranslate).toHaveBeenCalledWith(
      'settings.antivirus.scanProcess.detectedFiles'
    );
  ...
  it('updates the displayed counts when props change', () => {
    const { rerender } = render(
      <ScanStatistics scannedFilesCount={100} corruptedFilesCount={5} />
    );
    ...
    rerender(
      <ScanStatistics scannedFilesCount={200} corruptedFilesCount={10} />
    );
  ...
  it('applies the correct styling', () => {
    ...
    expect(container).toHaveClass(
      'flex h-full w-full items-stretch gap-5 rounded-xl bg-surface py-4'
    );

    const innerContainer = screen
      .getByText('100')
      .closest('div.flex.w-full.flex-row');
    expect(innerContainer).toHaveClass(
      'flex w-full flex-row justify-center gap-5'
    );

    const scannedContainer = screen
      .getByText('100')
      .closest('div.flex.w-full.max-w-\\[124px\\]');
    ...
    const rowContainer = screen
      ...
      .closest('div.flex.w-full.flex-row');
    const divider =
      rowContainer && rowContainer.querySelector('div.flex.flex-col.border');
    expect(divider).not.toBeNull();
    expect(divider).toHaveClass('flex flex-col border border-gray-10');

    const corruptedContainer = screen
      .getByText('5')
      .closest('div.flex.w-full.max-w-\\[124px\\]');
