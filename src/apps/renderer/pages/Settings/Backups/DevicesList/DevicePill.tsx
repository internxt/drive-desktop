...
const DevicePill: FC<DevicePillProps> = ({ device, current, selected, setSelected }) => {
  ...
  return (
    <div className={styles} onClick={() => setSelected(device)} data-testid={`device-pill-${device.id}`}>
      {current && <div className="text-xs text-primary">{translate('settings.backups.this-device')}</div>}
      {device.name}
    </div>
  );
};
