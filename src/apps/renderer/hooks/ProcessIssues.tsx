...
export default function useVirtualDriveIssues() {
  const [virtualDriveIssues, setVirtualDriveIssues] = useState<VirtualDriveIssue[]>([]);

  useEffect(() => {
    window.electron.getVirtualDriveIssues().then(setVirtualDriveIssues);
    const removeListener = window.electron.onProcessIssuesChanged(setVirtualDriveIssues);
