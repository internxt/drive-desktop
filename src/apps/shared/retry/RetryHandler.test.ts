...
describe('RetryHandler', () => {
  ...
  it('should retry the function until it succeeds', async () => {
    const mockFn = vi.fn().mockRejectedValueOnce(new Error('First attempt failed')).mockResolvedValueOnce('Success');
  ...
  it('should not retry if shouldRetry returns false', async () => {
    ...
    const mockFn = vi.fn().mockRejectedValue(new Error('Non-retriable error'));
