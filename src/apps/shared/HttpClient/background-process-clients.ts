...
export function getClients(): AuthorizedClients {
  if (!clients) {
    clients = {
      drive: new AuthorizedHttpClient(driveHeadersProvider, onUserUnauthorized, syncBlockedTracker).client,
      newDrive: new AuthorizedHttpClient(newDriveHeadersProvider, onUserUnauthorized, syncBlockedTracker).client,
    };
  }
