...
ipcMain.handle('create-file', async (_, body: components['schemas']['CreateFileDto']) => {
  return await createFile(body);
});

ipcMain.handle('move-file', async (_, params: { uuid: string; destinationFolder: string }) => {
  return await moveFile(params);
});

ipcMain.handle('rename-file', async (_, params: { plainName: string; type: string; folderUuid: string }) => {
  return await renameFile(params);
});

ipcMain.handle('delete-file-content', async (_, params: { bucketId: string; fileId: string }) => {
