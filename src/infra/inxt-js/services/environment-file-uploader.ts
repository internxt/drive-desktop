...
export class EnvironmentFileUploaderError extends Error {
  constructor(public readonly cause: 'KILLED_BY_USER' | 'NOT_ENOUGH_SPACE' | 'UNKNOWN') {
    super();
  }
}
        ...
        finishedCallback: (err: Error | null, contentsId: string) => {
          ...
          if (err) {
            if (err.message === 'Process killed by user') {
              return resolve({ error: new EnvironmentFileUploaderError('KILLED_BY_USER') });
              ...
              return resolve({ error: new EnvironmentFileUploaderError('NOT_ENOUGH_SPACE') });
            } else {
              logger.error({
              ...
              return resolve({ error: new EnvironmentFileUploaderError('UNKNOWN') });
            }
          }
