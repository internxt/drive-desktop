...
export class FileBatchUploader {
  constructor(
    private readonly fileUploader: EnvironmentFileUploader,
    private readonly creator: SimpleFileCreator,
  ) {}

  async run(context: BackupsContext, remoteTree: RemoteTree, batch: Array<LocalFile>, updateProgress: () => void): Promise<void> {
    ...
    for (const chunk of chunks) {
      await Promise.all(
        chunk.map(async (localFile) => {
          try {
            const { data: contentsId, error } = await this.fileUploader.upload({
            ...
            if (error) {
              ...
              addBackupsIssue({ name: localFile.relativePath, error: error.cause });
            } else {
            ...
            addBackupsIssue({ name: localFile.relativePath, error: 'UNKNOWN' });
          } finally {
            updateProgress();
          }
        }),
