	...
	const dropdown = (
		...
		>
			<Menu.Items className="absolute right-0 top-5 w-32 origin-top-right rounded-md bg-white py-1 shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none">
				...
				<Menu.Item>
					{({ active }) => (
						<div role="button" tabIndex={0} aria-hidden="true" onClick={window.electron.logout}>
							<DropdownItem active={active}>
...
function AccountSection() {
	...
	if (rawUsage === 'loading') {
		usageDisplayElement = <p className="text-xs text-neutral-500/80">Loading...</p>;
	} else if (rawUsage === 'error') {
		usageDisplayElement = <p />;
	} else {
		usageDisplayElement = (
			<p className="text-xs text-neutral-500">{`${bytes.format(rawUsage.usageInBytes)} ${translate(
				'widget.header.usage.of'
			)} ${rawUsage.isInfinite ? '∞' : bytes.format(rawUsage.limitInBytes)}`}</p>
		);
	}
