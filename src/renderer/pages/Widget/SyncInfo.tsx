...
import { BackupExitReason, BackupProgress } from '../../../main/background-processes/backups';
import { SyncStatus } from '../../../main/background-processes/sync';
import { ProcessErrorName, ProcessInfoUpdatePayload } from '../../../workers/types';
import Error from '../../assets/error.svg';
import FileIcon from '../../assets/file.svg';
import Success from '../../assets/success.svg';
import Warn from '../../assets/warn.svg';
import FileWithOperation, { Operation } from '../../components/FileWithOperation';
...
export default function SyncInfo() {
	...
	function onSyncItem(item: ProcessInfoUpdatePayload) {
		...
		setItems((currentItems) => {
			...
			const itemIsLocalDelete =
				(item.action === 'DELETE' || item.action === 'DELETED') && item.kind === 'LOCAL';
	...
	function clearItems() {
		setItems((current) =>
			current.filter((item) => ['PULL', 'RENAME', 'DELETE'].includes(item.action))
		);
	}
	...
	function removeOnProgressItems() {
		setItems((currentItems) => {
			return currentItems.filter(
				(item) => item.action !== 'DELETE' && item.action !== 'PULL' && item.action !== 'RENAME'
			);
		});
	}
...
function AnimationWrapper({ children, key, i }: { children: ReactNode; key: string; i: number }) {
	return (
		<motion.div
			key={key}
			initial={{ opacity: 0 }}
...
function Item({
	...
	return (
		<div className="my-4 flex h-10 w-full select-none items-center overflow-hidden px-3">
			<FileWithOperation operation={operation} className="flex-shrink-0" width={24} />
			<div className="ml-4 overflow-hidden">
				<h2 className="truncate text-sm font-medium text-neutral-700">{displayName}</h2>
		...
		<AnimatePresence>
			...
			>
				<div className="trasform absolute left-1/2 top-1/2 w-full -translate-x-1/2 -translate-y-1/2 select-none text-center">
					...
					<p className="mt-7 text-sm text-blue-100">{translate('widget.body.no-activity.title')}</p>
...
function BackupsBanner({
	...
	const issues = useProcessIssues().filter((issue) => issue.process === 'BACKUPS');
...
function BackupsIcon({ variant }: { variant?: 'SUCCESS' | 'WARNING' | 'ERROR' }) {
	return (
		<div className="relative">
			...
			{variant === 'WARNING' && <Warn className="absolute bottom-0 right-0 h-3 w-3 " />}
			{variant === 'ERROR' && <Error className="absolute bottom-0 right-0 h-3 w-3 " />}
		</div>
	);
}
