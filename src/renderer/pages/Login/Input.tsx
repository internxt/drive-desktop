...
export default function Input({
	...
	if (state === 'ready') {
		inputColors = 'border-m-neutral-60 focus:border-blue-50 focus:ring-2 focus:ring-blue-20';
	} else if (state === 'error') {
	...
	return (
		<div>
			<p className={`text-xs font-medium tracking-wide ${className} ${labelColors}`}>{label}</p>
			<div className="relative mt-1">
				...
				{type === 'password' && isFocused && (
					...
					>
						{translate(showPassword ? 'login.password.hide' : 'login.password.show')}
					</div>
				)}
			</div>
		</div>
