...
export default function BackupsSection({ active }: { active: boolean }) {
	...
	if (deviceState.status === 'LOADING') {
		...
		content = (
			<div className="flex h-32 items-center justify-center">
				<p className="text-sm text-red-60">There was an error loading your backups</p>
			</div>
		);
	} else {
		content = (
			<>
				{subsection === 'panel' && <BackupsPanel onGoToList={() => setSubsection('list')} />}
				{subsection === 'list' && <BackupsList onGoToPanel={() => setSubsection('panel')} />}
			</>
		);
	}
