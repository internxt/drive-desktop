					...
					>
						<motion.div
							variants={{
								INITIAL: { height },
								REPORTING: {
									height: stateIsError(requestState) ? '305px' : reportingPhaseHeight,
								},
							}}
							animate={phase}
						...
						>
							<div className="p-5" ref={measuredRef}>
								<Dialog.Title as="h3" className="font-medium leading-6 text-gray-90">
									{dialogTitle}
								</Dialog.Title>
								<div className="mt-2">
								...
								{phase === 'REPORTING' && (
									<>
										...
										{stateIsError(requestState) && (
											<p className="mt-4 text-xs text-red-60">{errorMessages[requestState]}</p>
										)}
									</>
								)}
								...
								<div className="mt-4 flex items-center justify-end space-x-2">
									<button
										type="button"
										onClick={() => (phase === 'INITIAL' ? onClose() : setPhase('INITIAL'))}
									...
									>
										{phase === 'INITIAL' ? (
											...
											<Spinner className="mx-auto animate-spin fill-white" width="18" height="18" />
										) : (
