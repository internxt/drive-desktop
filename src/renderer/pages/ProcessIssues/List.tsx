...
export default function ProcessIssuesList({
	...
	onClickOnErrorInfo: (errorClicked: Pick<ProcessIssue, 'errorName' | 'errorDetails'>) => void;
}) {
	const { translate } = useTranslationContext();
	const [isLoading, setIsLoading] = useState(false);
	const fatalErrorActionMap = useFatalErrorActions(showBackupFatalErrors ? 'BACKUPS' : 'SYNC');
	...
	function onInfoClick(errorName: ProcessErrorName) {
		onClickOnErrorInfo({
			errorName,
			errorDetails: processIssues.find((i) => i.errorName === errorName)!.errorDetails,
		});
	}
		...
		if (selectedTab === 'GENERAL') {
			return generalIssues.map((issue) => {
				...
				return (
					<GeneralIssueItem
						onClick={() =>
							selectedErrorName === error ? setSelectedErrorName(null) : setSelectedErrorName(error)
						}
						key={error}
						errorName={error}
	...
	const actionWrapper = (action: Action) => async (error: BackupFatalError | undefined) => {
		setIsLoading(true);
			...
			{showBackupFatalErrors &&
				backupFatalErrors.map((error) => (
					<FatalError
						...
						onActionClick={() => actionWrapper(fatalErrorActionMap[error.errorName])(error)}
					/>
				))}
			{renderItems()}
...
function Empty() {
	...
	return (
		<div className="flex h-full select-none items-center justify-center">
			<p className="text-xs font-medium text-m-neutral-60">{translate('issues.no-issues')}</p>
		</div>
	);
}
		...
		>
			<div className="flex items-center">
				...
				<div className="flex-grow">
					<h1 className="font-semibold text-gray-70" data-test="sync-issue-name">
				...
				<UilAngleDown
					className={`h-4 w-4 transform transition-all ${isSelected ? 'rotate-180' : 'rotate-0'}`}
				/>
			</div>
			<AnimatePresence>
				{isSelected && (
		...
		>
			...
			<AnimatePresence>
				{isSelected && (
					...
					>
						{issues.map((issue) => (
							<div className="mt-2 flex min-w-0 items-center overflow-hidden" key={issue.name}>
								<FileIcon className="h-5 w-5 flex-shrink-0" />
								<p className="ml-2 flex-grow truncate text-gray-70">{getBaseName(issue.name)}</p>
							</div>
						))}
					</motion.div>
				)}
	...
	return (
		<div className="select-none p-2 hover:bg-l-neutral-10 active:bg-l-neutral-20">
			<div className="flex items-center">
				...
				<div className="flex-grow">
					<h1 className="font-semibold text-gray-70">{window.electron.path.basename(path)}</h1>
