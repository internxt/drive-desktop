        ...
        database
          ...
          .then(result => {
            if (result === true) {
              ...
              database.dbFiles.remove(
                {},
                { multi: true },
                (err, totalFilesRemoved) => {
                  if (err) {
                    next(err)
                  } else {
                    database.dbFolders.remove(
                      {},
                      { multi: true },
                      (err, totalFoldersRemoved) => next(err)
                    )
                  }
                }
              )
...
function UploadNewFolders() {
  ...
  return new Promise((resolve, reject) => {
    Downloader.UploadAllNewFolders()
      .then(() => resolve())
      .catch(err => reject(err))
  })
}
