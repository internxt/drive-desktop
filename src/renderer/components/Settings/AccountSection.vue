    ...
    <div v-if="user" class="flex items-center justify-between">
      <div class="flex items-center">
        ...
        <div class="ml-4">
          ...
          <p class="tracking-wide text-sm text-gray-500">{{ user.email }}</p>
        </div>
      </div>
      <Button @click="logout">Log out</Button>
    </div>
    <div class="p-4 mt-5 bg-gray-100 rounded-xl" v-if="limit && usage">
      <div class="flex justify-between items-center">
        <div>
          <div class="flex items-center">
            <p class="mr-2 tracking-wide text-gray-600 text-sm">
              Current Plan
            </p>
        ...
        <Button v-if="!itsUnlimited" state="accent" @click="goToPricing"
          >Upgrade</Button
        >
      </div>
      ...
      <div class="flex justify-between align-center mt-2 text-xs">
        ...
        <p class="text-gray-500" v-if="!itsUnlimited">
          {{ percentageUsed }}% in use
        </p>
      </div>
    </div>
