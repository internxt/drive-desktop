...
import { syncBlocked, syncError, syncFinished, syncPaused, syncStarted } from './service';
...
ipcMain.on('SYNC_INFO_UPDATE', (_, data) => {
	if (data.errorName) {
		syncProgressInfo.errors.push(`${data.errorName} error when ${data.errorDetails.action}`);
