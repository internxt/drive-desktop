...
ipcMain.handle('get-last-backup-timestamp', () => configStore.get('lastBackup'));
...
function scheduleBackups(milliseconds: number) {
	backupsProcessRerun = setTimeout(() => startBackupProcess(true), milliseconds);
}
...
async function startBackupProcess(scheduled: boolean) {
	...
	backupsLastExitReason = hasBeenStopped.value ? 'FORCED_BY_USER' : 'PROCESS_FINISHED';
...
function processBackupsItem(item: BackupsArgs, hasBeenStopped: { value: boolean }) {
	return new Promise<void>((resolve) => {
		...
		function onExit(
			...
			Logger.log(`[onBackupsExit] ${item.path} (${item.folderId}) reason: ${payload.reason}`);
		...
		onExitFuncs.push(() => ipcMain.removeListener('stop-backups-process', onUserStopped));
	});
}
...
eventBus.on('WIDGET_IS_READY', () => {
	...
	if (lastBackup !== -1 && backupsInterval !== -1) {
		...
		const millisecondsToNextBackup = lastBackup + backupsInterval - currentTimestamp;
