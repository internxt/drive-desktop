...
import { FolderIsLocked, LockServiceUnavailabe, UnknonwLockServiceError } from './lock-erros';

async function acquireOrRefreshLock(folderId: number, lockId: string): Promise<'OK' | never> {
...
async function releaseLock(folderId: number, lockId: string): Promise<'OK' | never> {
	const res = await fetch(`${process.env.API_URL}/api/storage/folder/${folderId}/lock/${lockId}`, {
		method: 'DELETE',
		headers: getHeaders(),
	});
