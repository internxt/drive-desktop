...
export async function getOrCreateDevice() {
	...
	if (deviceIsDefined) {
		const res = await fetch(`${process.env.API_URL}/api/backup/deviceAsFolder/${savedDeviceId}`, {
			method: 'GET',
			headers: getHeaders(),
		});
...
export async function renameDevice(deviceName: string): Promise<Device> {
	...
	const res = await fetch(`${process.env.API_URL}/api/backup/deviceAsFolder/${deviceId}`, {
		method: 'PATCH',
		headers: getHeaders(true),
		body: JSON.stringify({ deviceName }),
	});
...
export async function getBackupsFromDevice(): Promise<(Backup & { pathname: string })[]> {
	const deviceId = getDeviceId();
...
async function fetchFolder(folderId: number) {
	const res = await fetch(`${process.env.API_URL}/api/storage/v2/folder/${folderId}`, {
		method: 'GET',
		headers: getHeaders(true),
	});
...
export async function deleteBackup(backup: Backup): Promise<void> {
	const res = await fetch(`${process.env.API_URL}/api/storage/folder/${backup.id}`, {
		method: 'DELETE',
		headers: getHeaders(true),
	});
	if (!res.ok) {
	...
	const entriesFiltered = Object.entries(backupsList).filter(([, b]) => b.folderId !== backup.id);
...
async function getPathFromDialog(): Promise<string | null> {
	...
	return chosenPath + (chosenPath[chosenPath.length - 1] === path.sep ? '' : path.sep);
}
