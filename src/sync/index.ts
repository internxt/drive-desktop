...
import Logger from '../libs/logger'
  ...
  ipcRenderer.invoke('get-sync-details').then(async ({localPath, folderId, remotePath}: {localPath: string, folderId: number, remotePath: string}) => {
    ...
    sync.on('PULLING_FILE', (name, progress, kind) => {
      ...
      app.emit('SYNC_INFO_UPDATE', {remotePath, localPath, action: 'PULL', kind, progress, name})
    })
    ...
    sync.on('RENAMING_FILE', (oldName, newName, kind) => {
      ...
      app.emit('SYNC_INFO_UPDATE', {remotePath, localPath, action: 'RENAME', kind, name: oldName})
    })
    ...
    sync.on('ERROR_RENAMING_FILE', (oldName, newName, kind) =>
      Logger.log(`Error renaming file from ${oldName} to ${newName} in ${kind}`)
    )

    sync.on('DELETING_FILE', (name, kind) => {
      Logger.log(`Deleting file ${name} in ${kind}`)
      app.emit('SYNC_INFO_UPDATE', {remotePath, localPath, action: 'DELETE', kind, name})
    })
    ...
    sync.on('FINALIZING', () => {
      ...
      app.emit('SYNC_INFO_UPDATE', {remotePath, localPath, action: 'FINALIZE'})
    })
