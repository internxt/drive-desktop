...
(async function () {
  ...
  sync.on('ERROR_PULLING_FILE', (name, kind) => console.log(`Error pulling file ${name} from ${kind}`))
  ...
  sync.on('ERROR_RENAMING_FILE', (oldName, newName, kind) => console.log(`Error renaming file from ${oldName} to ${newName} in ${kind}`))
  sync.on('DELETING_FILE', (name, kind) => console.log(`Deleting file ${name} in ${kind}`))
  sync.on('ERROR_DELETING_FILE', (name, kind) => console.log(`Error deleting file ${name} in ${kind}`))
  ...
  sync.on('ERROR_DELETING_FOLDER', (name, kind) => console.log(`Error deleting folder ${name} in ${kind}`))
